...
export function addGrid(elements: VisualElement[], scale: number, xAxis: string, xLabels: string[], yAxis: string, yLabels: string[], color?: Color, rotated = false) {
    ...
    for (let i = 0; i < xLabels.length; i++) {
        ...
        elements.push(new Text({ position, text: xLabels[i], horizontalAlignment: rotated ? 'right' : 'middle', verticalAlignment: rotated ? 'middle' : 'top', color, transform: rotated ? rotate(position, -90) : undefined }));
    }
