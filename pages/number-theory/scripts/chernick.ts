...
for (const n of chernicksCarmichaelNumbers) {
    ...
    console.log(`| ${encodeIntegerWithoutStore(k)} | [${encoding}](#tool-integer-fermat-primality-test&input=${escaped}&candidates=&rounds=100) | [${factors.map(factor => encodeIntegerWithoutStore(factor.base)).join(' · ')}](#tool-integer-factorization-trial-division&integer=${escaped}) | ${rate.toFixed(digits)} | ${(rate ** 100).toFixed(digits)}`);
}
