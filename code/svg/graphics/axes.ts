...
export function addAxes(elements: VisualElement[], scale: number, topLeft: Point, bottomRight: Point, color?: Color, labels = true) {
    ...
    if (labels) {
        elements.push(new Text({ position: P(bottomRight.x - textToLineDistance, -textToLineDistance), text: bold('x'), horizontalAlignment: 'right', verticalAlignment: 'bottom', color }));
        elements.push(new Text({ position: P(-textToLineDistance, topLeft.y + textToLineDistance), text: bold('y'), horizontalAlignment: 'right', verticalAlignment: 'top', color }));
    }
