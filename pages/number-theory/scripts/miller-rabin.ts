...
// This script generates the Markdown table at /number-theory/#choice-of-bases.
// You can run it with `npx ts-node pages/number-theory/scripts/miller-rabin.ts` from the project directory.

console.log('| l | Bases | Smallest composite integer for which the test fails');
console.log('|-:|:-|-:');
...
for (let i = 1; i <= failures.length; i++) {
    const bases = primes.slice(0, i).join(', ');
    const failure = encodeIntegerWithoutStore(failures[i - 1]);
    console.log(`| ${i} | [${bases}](#tool-integer-miller-rabin-primality-test&bases=${encodePercent(bases)}) | [${failure}](#tool-integer-miller-rabin-primality-test&input=${encodePercent(failure)}&bases=${encodePercent(bases)}&rounds=0) > 2<sup>${failures[i - 1].toString(2).length - 1}</sup>`);
}
